    ...
    <Link href={`/opportunities/${opportunity.id}`}>
      <Card className="h-full hover:shadow-lg transition-all duration-300 hover:scale-[1.02] cursor-pointer group bg-card">
        <CardHeader className="pb-3">
          ...
          <CardTitle className="text-lg line-clamp-2 group-hover:text-primary transition-colors text-card-foreground">
            {opportunity.title}
          </CardTitle>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            {opportunity.growthRate && (
              <div>
                ...
                <p className="text-green-400 font-medium">{opportunity.growthRate}</p>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-border">
            ...
            <Button
              ...
              onClick={handleSpark}
              disabled={sparking}
              className={`flex items-center gap-1 ${sparked ? 'text-primary' : 'text-card-foreground'} hover:text-primary`}
            >
